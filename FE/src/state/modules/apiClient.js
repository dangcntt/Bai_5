...
class ApiClient{
    ...
    async get(url, config = null) {
        try {
            ...
            console.log("LOG GET : " , url,response)
            ...
            if (e.toString().includes("401"))
            {
                return {
                    success: false,
                    code: 401,
                    message: "Bạn đã hết phiên đăng nhập vui lòng đăng nhập lại tiếp tục phiên giao dịch !",
                };
            }
            return {
                success: false,
    ...
    async post(url, data, config = null) {
        try {
          ...
          //  console.log("LOG POST DELETE " , url , e.toString())
            if (e.toString().includes("401"))
            {
                console.log("LOG IF " , url , e.toString())
                return {
                    success: false,
                    code: 401,
                    message: "Bạn đã hết phiên đăng nhập vui lòng đăng nhập lại tiếp tục phiên giao dịch !",
